...
namespace LinkingSystemTransformer
{
    public static class Program
    {
        ...
        public static void Main()
        {
            ...
            try
            {
                ...
                xmlSerializer = new XmlSerializer(typeof(Collection));
                ...
                {
                    PersonHashtable castAndCrewHash;
                    ...
                    {
                        using(StreamWriter sw = new StreamWriter(fs, Encoding.GetEncoding("Windows-1252")))
                        {
                                    ...
                                    foreach (Object possibleCast in dvd.CastList)
                                    {
                                        ...
                                        if(cast != null)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                name = ParsePersonName(cast.CreditedAs);
        ...
        private static void FillDynamicHash<T>(PersonHashtable personHash, Object possiblePerson) where T : class, IPerson
        {
            T person;
    ...
    internal class Hashtable<TKey> : Dictionary<TKey, Int32>
    {
        ...
        {
            base.Add(key, Program.s_IdCounter++);
        }
    }
    ...
    internal class PersonKey
    {
        ...
        {
            return (m_Person.LastName.GetHashCode() / 4 + m_Person.FirstName.GetHashCode() / 4
                + m_Person.MiddleName.GetHashCode() / 4 + m_Person.BirthYear.GetHashCode() / 4);
        ...
        public override Boolean Equals(Object obj)
        {
            ...
            {
                return (m_Person.LastName == other.Person.LastName && m_Person.FirstName == other.Person.FirstName
                    && m_Person.MiddleName == other.Person.MiddleName && m_Person.BirthYear == other.Person.BirthYear);
            }
        }
        ...
        public override String ToString()
        {
            ...
            if(String.IsNullOrEmpty(this.m_Person.FirstName) == false)
            {
                name.Append("<" + this.m_Person.FirstName + ">");
            }
            if(String.IsNullOrEmpty(this.m_Person.MiddleName) == false)
            {
                if(name.Length != 0)
                {
                    name.Append(" ");
                }
                name.Append("{" + this.m_Person.MiddleName + "}");
            }
            if(String.IsNullOrEmpty(this.m_Person.LastName) == false)
            {
                if(name.Length != 0)
                {
                    name.Append(" ");
                }
                name.Append("[" + this.m_Person.LastName + "]");
            }
            if(this.m_Person.BirthYear != 0)
            {
                if(name.Length != 0)
                {
                    name.Append(" ");
                }
                name.Append("(" + this.m_Person.BirthYear + ")");
            }
